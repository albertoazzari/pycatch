...
use crate::statistics::{
    self, autocorr, autocorr_lag, autocov_lag, coarsegrain, covariance_matrix, diff, f_entropy, first_zero, histbinassign, histcount_edges, histcounts, is_constant, linreg, max_, mean, median, min_, norm, num_bins_auto, splinefit, std_dev, welch
};
use pyo3::pyfunction;
...
pub fn compute(x: Vec<f64>, n: usize) -> f64 {
    match n {
        0 => dn_outlier_include_np_001_mdrmd(&x, false),
        1 => dn_outlier_include_np_001_mdrmd(&x, true),
        2 => dn_histogram_mode_n(&x, 5),
        3 => dn_histogram_mode_n(&x, 10),
        4 => co_embed2_dist_tau_d_expfit_meandiff(&x),
        5 => co_f1ecac(&x),
        6 => co_first_min_ac(&x),
        7 => co_histogram_ami_even_tau_bins(&x, 2, 5),
        8 => co_trev_1_num(&x),
        9 => fc_local_simple_mean_tauresrat(&x, 1),
        10 => fc_local_simple_mean_stderr(&x, 3),
        11 => in_auto_mutual_info_stats_tau_gaussian_fmmi(&x, 40.0),
        12 => md_hrv_classic_pnn(&x, 40),
        13 => sb_binary_stats_diff_longstretch0(&x),
        14 => sb_binary_stats_mean_longstretch1(&x),
        15 => sb_motif_three_quantile_hh(&x),
        16 => sc_fluct_anal_2_50_1_logi_prop_r1(&x, 1, "rsrangefit"),
        17 => sc_fluct_anal_2_50_1_logi_prop_r1(&x, 2, "dfa"),
        18 => sp_summaries_welch_rect(&x, "area_5_1"),
        19 => sp_summaries_welch_rect(&x, "centroid"),
        20 => sb_transition_matrix_3ac_sumdiagcov(&x),
        21 => pd_periodicity_wang_th0_01(&x),
